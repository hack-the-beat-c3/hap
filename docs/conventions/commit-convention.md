...
- 제목과 본문, 본문과 꼬리말 사이는 한 줄 띄웁니다.
- **본문과 꼬리말은 가급적 사용하지 않습니다.** 제목만으로 설명이 부족한 정말 필요한 경우에만 사용합니다.
...
- **style**: 코드 포맷/스타일 수정 (코드 로직 변경 없음)
  - _예) style(imports): eslint 규칙에 맞게 import 정렬_
...
- **꼬리말(Footer):** 이슈 참조(`Closes #7`), 호환성 깨지는 변경(`BREAKING CHANGE:`), 공동 작성자(`Co-Authored-By:`) 등 정말 필요한 경우에만 적습니다. (가급적 생략)